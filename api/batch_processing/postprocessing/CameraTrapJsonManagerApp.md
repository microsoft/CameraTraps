...
This is a Windows desktop application for making the batch processing API's output JSON file more amenable to downstream processing. It does either or both of the following

- Retrieves all result entries (each result entry corresponds to one image, containing all detections on it) where the image file path matches a specified query string. It optionally replaces that query string with a replacement token. If the query string is blank, it can be used to prepend a token to all image file paths. 

- Splits the batch processing API's output JSON file into smaller files each containing only results corresponding to a subfolder of images. 
...
|--------------------------|----------------------------|
| Input file               | Path to the output JSON file of the batch processing API. |
...
| Query                    | Specify a query string to retrieve  entries with image file path containing this string. Leave blank to retrieve all entries. <br/> <br/> Example: <br/>Specify `Unprocessed Images/Camera 3/` to restrict to images from this folder. <br/>Specify `Location 1` to retrieve all image files that have `Location 1` in its path. <br/>Remember to use forward slash "/" here since it is used in the API's output file. Regular expression is not supported.
| Replacement              | A string to replace the Query string in the image file paths. If Query is left blank, the Replacement string will be prepended to all image file paths.
| Confidence threshold     | Only detections with confidence above this threshold will be copied over to the resulting JSON file(s).
...
| Copy jsons to folders    | If "Split folders" and "Make folder-relative" are checked, checking this option will copy the resulting small JSON file to the subfolder containing the corresponding images. |
...
| Overwrite json files     | Check to overwrite output files if they already exist. |
| Make folder-relative     | Check to make the image file paths in the resulting JSON(s) relative to the their containing folder. Only meaningful if "Split folders" is checked. |


###"Split folder mode" explained:

Running example - let's say the image file paths in your API output are like this:

```
Unprocessed Images/NE4311/Camera_4/C128_11.11.19/DCIM/105RECNX/RCNX1111.JPG
...
```
Unprocessed Images/NE4311/Camera_4/C128_11.11.19/DCIM/105RECNX
...
What if you want each JSON file to represent a subfolder that's not the top-most nor the bottom-most folders?
...
`N` of "0" would the same as "Top" (top-most folders), "1" would be one more level down, and "2" would result in one JSON for each of
```
...
Here `N` of "0" would the same as "Bottom" (bottom-most folders), "1" would be one level up, and "2" would result in one JSON for each of 
```
Unprocessed Images/NE4311/Camera_4/C128_11.11.19
...
Hit `Process` to proceed. The output window will show a process bar and the paths to the resulting, smaller JSON files. 

Clicking the `Help` button will open this page.



## Other notes

You should not need to worry about the fact that the output file uses forward slashes "/"; both the Output Manager and Timelapse will handle the path separator correctly.
