...
dataset_name = 'mdv4boxes'  # 'mdv4boxes' or 'mdv4empty'
assert len(dataset_name) == 9

split_name = 'test_'  # 'train', 'val__' or 'test_'
assert len(split_name) == 5

image_dir = '/beaver_disk/camtrap/mdv4_splitted/test'
assert split_name.replace('_', '') in image_dir

annotations_path = '/data/home/marmot/camtrap/PyCharm/CameraTraps/temp_data/bboxes_20200123.json'
...
with open(os.path.join(tfrecord_dir, '{}_failed.json'.format(split_name)), 'w') as f:
