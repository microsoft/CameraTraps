...
def main(
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        save_top_k=1,
        filename="{}-{}".format(conf.conf_id, session)
        + "-{epoch:02d}-{valid_mac_acc:.2f}",
        verbose=True,
    )
    ...
    if evaluate is not None:
        if val:
            trainer.validate(
                learner, dataloaders=[dataset.val_dataloader()], ckpt_path=evaluate
            )
        elif predict:
            trainer.predict(
                learner, dataloaders=[dataset.predict_dataloader()], ckpt_path=evaluate
            )
        else:
            trainer.test(
                learner, dataloaders=[dataset.test_dataloader()], ckpt_path=evaluate
            )
    else:
